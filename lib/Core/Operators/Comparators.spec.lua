...
return function ()
	describe("Comparators", function()
		...
		it("should compare within a range", function()
			...
			expect(Constants.COMPARATOR_NEAR_DEFAULT > 0).to.equal(true)
			expect(Comparators.near(0, Constants.COMPARATOR_NEAR_DEFAULT/2)).to.equal(false)
			expect(Comparators.near(0, Constants.COMPARATOR_NEAR_DEFAULT*2)).to.equal(true)
		end)
	end)
